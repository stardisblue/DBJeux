...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Items User'), ['action' => 'edit', $itemsUser->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Items User'), ['action' => 'delete', $itemsUser->id], ['confirm' => __('Are you sure you want to delete # {0}?', $itemsUser->item_id)]) ?> </li>
...
<div class="itemsUsers view large-9 medium-8 columns content">
    <h3><?= h($itemsUser->item->author_title_type_owner . ' --- ' . $itemsUser->user->username) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Item') ?></th>
            <td><?= $itemsUser->has('item') ? $this->Html->link($itemsUser->item->author_title_type_owner, ['controller' => 'Items', 'action' => 'view', $itemsUser->item->id]) : '' ?></td>
        </tr>
        <tr>
